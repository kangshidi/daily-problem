...
9. 元素有 -webkit-overflow-scrolling 值且值为 touch. <br />
还有少数几种冷门的情况
...
### 21. 修复下拉菜单闪屏问题。
原本项目中顶部菜单采用的是antd dropdown组件实现的。<br />
当鼠标移动到菜单标题上就立刻展示下拉框，导致鼠标在菜单标题上移动的时候，下拉框收起来展开，收起来展开，下拉菜单又是半透明黑色底色，就会导致闪屏现象。<br />

想要达到的效果：<br />
1. 鼠标在菜单标题上来回移动的时候，下拉框不要收起来，只是替换下拉框中的菜单，移动到两个菜单标题中间的时候，下拉框收起。
2. 当下拉框中的菜单发生变化时需要有自适应高度的展开收起的动画效果。
3. 下拉框横向铺满浏览器。


解决方案：<br />
1. 自定义实现下拉菜单。监听菜单标题的mouseEnter（展示下拉）和mouseLeave事件（隐藏下拉），采用`防抖`来设置当前活动菜单（有值的时候展示下拉框，并且获取相应的下拉菜单数据）。<br />
为什么使用防抖？<br />
